  ...
  var _ref = _asyncToGenerator(function* (bundler, port, useHTTPS = false) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      server.once('listening', function () {
        let addon = server.address().port !== port ? `- ${bundler.logger.chalk.yellow(`configured port ${port} could not be used.`)}` : '';

        bundler.logger.persistent(`Server running at ${bundler.logger.chalk.cyan(`${useHTTPS ? 'https' : 'http'}://localhost:${server.address().port}`)} ${addon}`);
